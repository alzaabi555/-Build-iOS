...
// =================================================================================
// 1. محرك الطباعة (THE PRINT ENGINE) - لم يتم المساس بالمنطق الحساس
// =================================================================================
const PrintPreviewModal: React.FC<{ isOpen: boolean; onClose: () => void; title: string; content: React.ReactNode; landscape?: boolean; }> = ({ isOpen, onClose, title, content, landscape }) => {
    ...
    const handlePrint = async () => {
        ...
        if (!element) return;
        setIsPrinting(true);
        // إعادة التمرير للأعلى لضمان التقاط الصفحة كاملة
    ...
    return (
        <div className="fixed inset-0 z-[9999] bg-black/95 backdrop-blur-sm flex flex-col animate-in fade-in duration-200">
            <div className="bg-slate-900 text-white p-4 flex justify-between items-center border-b border-white/10 shrink-0 shadow-xl safe-area-top">
                <div className="flex items-center gap-3"><button onClick={onClose} className="p-2 hover:bg-white/10 rounded-full transition-colors"><ArrowRight className="w-6 h-6" /></button><div><h3 className="font-bold text-lg">{title}</h3><p className="text-xs text-slate-400 font-mono">{landscape ? 'A4 Landscape' : 'A4 Portrait'}</p></div></div>
                <button onClick={handlePrint} disabled={isPrinting} className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-2 rounded-xl font-bold flex items-center gap-2 shadow-lg disabled:opacity-50 transition-all active:scale-95">{isPrinting ? <Loader2 className="animate-spin w-5 h-5" /> : <Printer className="w-5 h-5" />} {isPrinting ? 'جاري المعالجة...' : 'تصدير ومشاركة'}</button>
            </div>
...
// =================================================================================
// 2. القوالب (TEMPLATES) - هذه هي "مناطق الطباعة" وهي محمية تماماً
// =================================================================================
    ...
    return (
        <div className="w-full text-black bg-white p-10 print-content">
            ...
            <table className="w-full border-collapse border border-black text-[10px]">
                <thead><tr className="bg-gray-200"><th className="border border-black p-1 w-8 text-center">م</th><th className="border border-black p-1 text-right w-48">الاسم</th>{tools.map((t: any) => <th key={t.id} className="border border-black p-1 bg-orange-50 text-center">{t.name}</th>)}<th className="border border-black p-1 bg-blue-100 text-center font-bold">المجموع (60)</th>{finalTool && <th className="border border-black p-1 bg-pink-100 text-center font-bold">{finalTool.name} (40)</th>}<th className="border border-black p-1 bg-gray-300 text-center font-black">الكلي</th><th className="border border-black p-1 text-center">الرمز</th></tr></thead>
                <tbody>{students.map((s: any, i: number) => { const semGrades = (s.grades || []).filter((g: any) => (g.semester || '1') === semester); let contSum = 0; const contCells = tools.map((tool: any) => { const g = semGrades.find((r: any) => r.category.trim() === tool.name.trim()); const val = g ? Number(g.score) : 0; contSum += val; return <td key={tool.id} className="border border-black p-1 text-center font-medium">{g ? g.score : '-'}</td>; }); const finalG = finalTool ? semGrades.find((r: any) => r.category.trim() === finalTool.name.trim()) : null; const finalVal = finalG ? Number(finalG.score) : 0; const total = contSum + finalVal; const getSymbol = (sc: number) => { if (sc >= 90) return 'أ'; if (sc >= 80) return 'ب'; if (sc >= 65) return 'ج'; if (sc >= 50) return 'د'; return 'هـ'; }; return (<tr key={s.id} style={{ pageBreakInside: 'avoid' }}><td className="border border-black p-1 text-center">{i + 1}</td><td className="border border-black p-1 font-bold whitespace-nowrap">{s.name}</td>{contCells}<td className="border border-black p-1 text-center font-bold bg-blue-50">{contSum}</td>{finalTool && <td className="border border-black p-1 text-center font-bold bg-pink-50">{finalG ? finalG.score : '-'}</td>}<td className="border border-black p-1 text-center font-black bg-gray-100">{total}</td><td className="border border-black p-1 text-center font-bold">{getSymbol(total)}</td></tr>); })}</tbody>
            </table>
