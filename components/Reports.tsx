...
// ✅ نافذة المعاينة مع إصلاح النوتش والطبقات
const PrintPreviewModal: React.FC<{ isOpen: boolean; onClose: () => void; title: string; content: React.ReactNode; landscape?: boolean; }> = ({ isOpen, onClose, title, content, landscape }) => {
    ...
    return (
        ...
        <div className="fixed inset-0 z-[99999] bg-black/95 backdrop-blur-sm flex flex-col animate-in fade-in duration-200 pt-12 md:pt-0">
            {/* زر إغلاق إضافي للهاتف */}
            <div className="absolute top-4 right-4 md:hidden z-50">
                <button onClick={onClose} className="p-2 bg-white/20 rounded-full text-white"><X/></button>
            </div>

            <div className="bg-slate-900 text-white p-4 flex justify-between items-center border-b border-white/10 shrink-0 shadow-xl relative z-40 rounded-t-2xl md:rounded-none">
                <div className="flex items-center gap-3">
                    <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-full transition-colors hidden md:block"><ArrowRight className="w-6 h-6" /></button>
            ...
            <div id="preview-scroll-container" className="flex-1 overflow-auto bg-slate-800 p-2 md:p-8 flex justify-center cursor-default">
...
const Reports: React.FC<ReportsProps> = ({ initialTab }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc] text-slate-800 relative font-sans animate-in fade-in duration-500">
      ...
      {/* جعلناه sticky ليأخذ مساحته الطبيعية ولا يغطي المحتوى ولا القائمة */}
