...
const StudentReport: React.FC<StudentReportProps> = ({ student }) => {
  ...
  const handlePrint = () => {
    ...
    // تنظيف أي إطارات قديمة
    const oldIframe = document.getElementById('print-iframe');
    if (oldIframe) document.body.removeChild(oldIframe);

    // 2. إنشاء إطار مخفي (Iframe)
    // ملاحظة للآيفون: يجب أن يكون للإطار حجم وموقع فعلي حتى لو كان خارج الشاشة ليتم رصده
    const iframe = document.createElement('iframe');
    iframe.id = 'print-iframe';
    iframe.style.position = 'absolute';
    iframe.style.left = '-10000px'; // إخفاء خارج الشاشة بدلاً من width:0
    iframe.style.top = '0';
    iframe.style.width = '1000px'; // عرض وهمي لضمان التنسيق
    ...
    if (doc) {
      ...
      doc.write(`
        ...
        <html dir="rtl" lang="ar">
          ...
          <body>
            ...
            <script>
              // تأخير لضمان تحميل Tailwind والصور
              window.onload = function() {
                setTimeout(function() {
                  try {
                    // التركيز على النافذة قبل الطباعة (مهم للآيفون)
                    window.focus();
                    window.print();
                  } catch(e) {
      ...
      // لا نحذف الإطار فوراً للسماح لعملية الطباعة بالاكتمال في الخلفية
    }
  };
