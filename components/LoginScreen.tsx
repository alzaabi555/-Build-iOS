...
// ✅ لاحظ: نستخدم signInWithPopup و signInWithRedirect
import { signInWithPopup, GoogleAuthProvider, signInWithCredential, signInWithRedirect, getRedirectResult } from 'firebase/auth';
...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLoginSuccess }) => {
  ...
  // محاولة التقاط العائدين من إعادة التوجيه (للويندوز والويب)
  useEffect(() => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      // 🛑 التغيير الجذري: ألغينا الشرط المعقد
      // سنجبر الجميع على استخدام الطريقة "البسيطة" (مثل الويندوز)
      // هذا يفتح نافذة متصفح داخل التطبيق، وهي مضمونة العمل
      ...
      // إذا فشل البوب-أب (نادر جداً)، نجرب إعادة التوجيه
      if (err?.code === 'auth/popup-blocked' || err?.code === 'auth/cancelled-popup-request') {
          try {
             await signInWithRedirect(auth, googleProvider);
             return; // سينتقل لصفحة أخرى
          } catch (redirectErr) {
             setError("فشل الدخول. الرجاء المحاولة مرة أخرى.");
          }
      } else {
         setError(`خطأ في الدخول: ${err.message}`);
      }
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-white relative">
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-8">
        ...
        {/* زر الطوارئ للدخول كزائر */}
        <button onClick={() => onLoginSuccess(null)} className="text-slate-400 font-bold text-xs hover:text-indigo-600 transition-colors">
          الدخول كزائر (بدون تسجيل)
        </button>

        {error && (
            <div className="mt-6 p-3 bg-rose-50 text-rose-600 text-xs font-bold rounded-xl border border-rose-100 flex items-center gap-2 justify-center w-full">
