...
import { Plus, X, Trash2, Settings, Check, Loader2, Edit2, FileSpreadsheet, FileUp, Wand2, Menu, Filter, ChevronDown, Download, AlertTriangle } from 'lucide-react';
...
const GradeBook: React.FC<GradeBookProps> = ({ 
  ...
  const [showMenuDropdown, setShowMenuDropdown] = useState(false); // القائمة المنسدلة

  const styles = {
      card: `
        bg-white border border-slate-100 rounded-[1.5rem] 
        hover:border-blue-200 hover:shadow-md 
        transition-all duration-300 relative overflow-hidden shadow-sm group
      `,
  ...
  return (
    <div className="flex flex-col h-full text-slate-800 bg-[#f8fafc] relative animate-in fade-in duration-500 font-sans">
        ...
        <div className="fixed top-0 left-0 right-0 z-50 bg-[#1e3a8a] text-white rounded-b-[2.5rem] shadow-lg px-4 pt-[env(safe-area-inset-top)] pb-6 transition-all duration-300">
            ...
            {/* Tools Strip (Horizontal Scroll for Quick Access) */}
            <div className="overflow-x-auto no-scrollbar flex gap-2 pt-1 pb-1">
                {tools.length > 0 ? tools.map(tool => (
                    <button 
                        key={tool.id}
                        onClick={() => { setBulkFillTool(tool); setBulkScore(''); }}
                        className="px-4 py-2 bg-white/10 backdrop-blur-sm rounded-xl text-[10px] font-bold text-white whitespace-nowrap hover:bg-white/20 border border-white/20 flex items-center gap-1.5 active:scale-95 transition-all"
                    >
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                                                {tools.slice(0, 3).map(tool => {
                                                    const grade = semGrades.find(g => g.category.trim() === tool.name.trim());
                                                    return (
                                                        <span key={tool.id} className="text-[9px] px-2 py-0.5 rounded-lg bg-slate-50 border border-slate-200 text-slate-600 font-bold">
        ...
        {/* Logic Preserved, Styling Updated */}
