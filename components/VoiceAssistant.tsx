...
const VoiceAssistant: React.FC<VoiceAssistantProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    // 💉 التعديل الأول: إيقاف النتائج المؤقتة لمنع التجمد وتأخير التنفيذ والتقاط الجملة فوراً
    ...
    recognition.onend = () => {
      if (shouldListenRef.current) {
        // 💉 التعديل الثاني: تأخير 350 جزء من الثانية قبل إعادة التشغيل لتجاوز الطنين الإجباري من أندرويد بصمت
        setTimeout(() => {
            if (shouldListenRef.current) {
