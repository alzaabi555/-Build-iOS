    ...
    <div className="space-y-6 pb-24 md:pb-8 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 rounded-[2rem] shadow-sm border border-gray-100">
             ...
             <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-1 custom-scrollbar">
                 <button onClick={() => setIsSetupMode(!isSetupMode)} className={`px-4 py-2 rounded-xl text-xs font-black flex items-center gap-2 transition-all ${isSetupMode ? 'bg-indigo-600 text-white shadow-lg' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}>
                     {isSetupMode ? <Unlock className="w-3.5 h-3.5" /> : <Lock className="w-3.5 h-3.5" />}
                     {isSetupMode ? 'إنهاء الإدارة' : 'إدارة الفرق'}
                 </button>
                 <div className="h-6 w-px bg-gray-200 mx-1"></div>
                 {classes.map(c => (<button key={c} onClick={() => setSelectedClass(c)} className={`px-3 py-2 rounded-xl text-[10px] font-black whitespace-nowrap transition-all ${selectedClass === c ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-500 border border-gray-100'}`}>{c}</button>))}
             </div>
        </div>
                ...
                {groups.map((group) => {
                    ...
                    return (
                        <div key={group.id} className={`relative bg-white rounded-[1.5rem] md:rounded-[2.5rem] p-3 md:p-5 shadow-sm border-2 transition-all duration-300 ${isLeader ? 'border-amber-400 scale-[1.02] shadow-xl shadow-amber-100' : 'border-gray-100'}`}>
                            {isLeader && (<div className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-amber-400 text-white px-2 py-0.5 rounded-full text-[9px] font-black flex items-center gap-1 shadow-sm z-10 whitespace-nowrap"><Crown className="w-2.5 h-2.5 fill-white" /> المتصدر</div>)}
                            <div className={`h-16 md:h-24 rounded-2xl md:rounded-[2rem] ${style.light} flex items-center justify-between px-3 md:px-6 mb-3 relative overflow-hidden`}>
                                ...
                                <div className="text-center z-10"><span className={`block text-xl md:text-3xl font-black ${style.text}`}>{score}</span><span className="text-[8px] md:text-[9px] font-bold text-gray-400 opacity-80">نقطة</span></div>
                            ...
                            <div className="grid grid-cols-2 gap-1 md:gap-2 mb-3">
                                <button onClick={() => awardTeam(group.id, 5, 'هدوء ونظام')} className="py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded-lg text-[9px] font-black transition-colors active:scale-95 flex flex-col items-center gap-0.5"><Plus className="w-3 h-3 md:w-4 md:h-4" /><span>نظام</span></button>
                                <button onClick={() => awardTeam(group.id, 10, 'إجابة جماعية')} className="py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-[9px] font-black transition-colors active:scale-95 flex flex-col items-center gap-0.5"><Zap className="w-3 h-3 md:w-4 md:h-4" /><span>تفاعل</span></button>
                                <button onClick={() => awardTeam(group.id, -5, 'إزعاج جماعي')} className="py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-lg text-[9px] font-black transition-colors active:scale-95 flex flex-col items-center gap-0.5 col-span-2"><Minus className="w-3 h-3 md:w-4 md:h-4" /><span>مخالفة</span></button>
                            </div>
                            <div className="bg-gray-50 rounded-xl p-2 md:p-3 min-h-[60px] md:min-h-[80px]"><div className="flex justify-between items-center mb-1 md:mb-2"><span className="text-[9px] md:text-[10px] font-bold text-gray-400">الأعضاء ({membersCount})</span><Users className="w-3 h-3 text-gray-300" /></div><div className="flex flex-wrap gap-1">{getTeamStudents(group.id).map(s => (<span key={s.id} className="text-[8px] md:text-[9px] bg-white border border-gray-100 px-1 py-0.5 rounded text-gray-600 truncate max-w-[50px] md:max-w-[60px]">{s.name.split(' ')[0]}</span>))}{membersCount === 0 && <span className="text-[8px] text-gray-300 mx-auto mt-1">لا يوجد أعضاء</span>}</div></div>
                        </div>
                    );
                })}
            </div>
        ...
        {isSetupMode && (
            <div className="bg-white rounded-[2.5rem] p-6 shadow-sm border border-gray-100">
                <div className="flex items-center gap-2 mb-6"><div className="bg-indigo-100 p-2 rounded-xl text-indigo-600"><RefreshCw className="w-5 h-5"/></div><div><h3 className="font-black text-gray-900">إدارة الفرق</h3><p className="text-xs text-gray-400 font-bold">اضغط على الفريق لتعديل الاسم وإضافة الطلاب</p></div></div>
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {groups.map(group => {
                        const style = getStyle(group.color);
                        const memberCount = getTeamStudents(group.id).length;
                        return (
                            <button key={group.id} onClick={() => openManageGroup(group)} className={`flex flex-col items-center justify-center p-6 rounded-[2rem] border-2 transition-all active:scale-95 hover:shadow-md ${style.light} ${style.border}`}>
                                ...
                                <span className="text-[10px] font-bold text-gray-500 bg-white/50 px-3 py-1 rounded-full">{memberCount} طالب</span>
                            </button>
                        );
                    })}
                </div>
        ...
        {/* Manage Group Modal - Compact Card */}
        <Modal isOpen={!!editingGroup} onClose={() => setEditingGroup(null)}>
            ...
            <div className="flex gap-2 mb-2 shrink-0">
                <input type="text" value={groupNameInput} onChange={e => setGroupNameInput(e.target.value)} className="flex-[2] bg-gray-50 border border-gray-100 rounded-xl px-3 py-3 text-sm font-bold outline-none focus:border-indigo-500" placeholder="اسم الفريق" />
                <button onClick={handleSaveGroup} className="flex-1 bg-indigo-600 text-white rounded-xl font-black text-xs shadow-lg shadow-indigo-200">حفظ</button>
            </div>

            <div className="flex gap-2 justify-center mb-2 shrink-0">{COLORS.map(c => (<button key={c.id} onClick={() => setGroupColorInput(c.id)} className={`w-8 h-8 rounded-full border-2 ${c.bg} ${groupColorInput === c.id ? 'border-gray-900 scale-110 shadow-sm' : 'border-transparent opacity-50'}`} />))}</div>

            <div className="flex-1 overflow-y-auto custom-scrollbar bg-gray-50 rounded-2xl p-2">
                <div className="p-2 pb-2"><div className="relative"><Search className="absolute right-3 top-3 w-4 h-4 text-gray-400" /><input type="text" placeholder="ابحث لإضافة طالب..." value={studentSearch} onChange={e => setStudentSearch(e.target.value)} className="w-full bg-white rounded-xl py-2.5 pr-9 pl-4 text-xs font-bold outline-none border border-gray-100 focus:border-indigo-200" /></div></div>
                <div className="space-y-1 p-1">
                    {filteredStudents.filter(s => s.name.toLowerCase().includes(studentSearch.toLowerCase())).map(student => {
                            ...
                            return (
                                <div key={student.id} onClick={() => toggleStudentGroup(student)} className={`p-2 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${inCurrentGroup ? 'bg-emerald-50 border-emerald-200' : 'bg-white border-gray-100'}`}>
                                    <div className="flex items-center gap-3"><div className={`w-5 h-5 rounded-full border flex items-center justify-center ${inCurrentGroup ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-gray-300 bg-white'}`}>{inCurrentGroup && <Check className="w-3 h-3" />}</div><p className={`text-xs font-black ${inCurrentGroup ? 'text-gray-900' : 'text-gray-600'}`}>{student.name}</p></div>
                                </div>
                            );
                        })
                    }
